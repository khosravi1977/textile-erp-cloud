...
function IncomingInvoicePage({ finance, setFinance, onlySource = '' }) {
  ...
  return (
    ...
    <div className="space-y-5">
      ...
      <Card><div className="mb-4 flex items-center justify-between"><h3 className="font-bold">{onlySource === 'chelle' ? 'لیست فاکتورهای ورود چله ثبت شده' : 'ليست فاکتورهاي ورود ثبت شده'}</h3><PrimaryButton onClick={printIncoming}>چاپ گزارش</PrimaryButton></div><IncomingInvoiceTable rows={rows} onEdit={edit} onDelete={remove} /></Card>
...
function CostsPage({ finance, setFinance }) {
  ...
  const saveExpense = e => {
    ...
    if (!form.group || !form.subgroup || !Number(form.amount || 0)) return;

    setFinance(prev => {

      const previousExpense = editingId ? prev.expenses.find(x => x.id === editingId) : null;

      const expense = { id: editingId || uid('exp'), ...form, amount: Number(form.amount), source: 'مالی' };

      const expenses = editingId ? prev.expenses.map(x => x.id === editingId ? expense : x) : [expense, ...prev.expenses];

      const movement = { id: uid('mov'), accountId: form.accountId, date: form.date, direction: 'out', amount: Number(form.amount), description: `هزینه: ${form.group} / ${form.subgroup}`, group: form.group, subgroup: form.subgroup, sourceExpense: expense.id };
  ...
  const deleteExpense = id => setFinance(prev => {
...
function AdvisorPage({ finance }) {
  ...
  return (
    ...
    <div className="space-y-5">
      ...
      <Card><div className="flex flex-wrap items-center justify-between gap-3"><h3 className="font-bold">تنظيم گزارش مشاور مالي</h3><div className="flex gap-2"><SelectInput value={months} onChange={e => setMonths(e.target.value)}><option value="1">۱ ماه</option><option value="2">۲ ماه</option><option value="3">۳ ماه</option><option value="6">۶ ماه</option></SelectInput><PrimaryButton onClick={printAdvisor}>چاپ گزارش مشاور</PrimaryButton></div></div></Card>

      <div className="grid grid-cols-4 gap-4"><Field label="وضعیت نقدینگی" value={money(cashBalance) + ' تومان'} tone={cashBalance >= payables ? 'text-emerald-300' : 'text-red-300'} /><Field label="شکاف نقدینگی با پیش‌بینی هزینه" value={money(liquidityGap) + ' تومان'} tone={liquidityGap >= 0 ? 'text-emerald-300' : 'text-red-300'} /><Field label="هزينه پيش بيني شده" value={money(forecastTotal) + ' تومان'} tone="text-amber-300" /><Field label="مانده مشتريان و واگذاري ها" value={money(totalDebt) + ' تومان'} tone="text-amber-300" /></div>
