...
func SetupRouter(services ...*telegramreport.Service) http.Handler {
	...
	mux.HandleFunc("/api/advisor/advice", h.GetAdviceAudited)
	mux.HandleFunc("/api/advisor/credit-report/", h.GetCreditReportAudited)
	...
	mux.HandleFunc("/api/costs/profitability", h.GetProfitabilityAudited)
	...
	// HesabYar typed financial core (v1 posting contract) and party sync.
	mux.HandleFunc("/api/v1/financial/transactions", h.TypedTransactionsRoot)
	mux.HandleFunc("/api/v1/financial/transactions/reverse", h.ReverseTypedTransaction)
	mux.HandleFunc("/api/v1/financial/parties/", h.PartyLedgerHandler)
	mux.HandleFunc("/api/v1/financial/bank-accounts", h.ListBankAccountsHandler)
	mux.HandleFunc("/api/parties", h.ListParties)
	// Device-authenticated (HesabYar Android) aliases of the same endpoints.
	mux.HandleFunc("/api/mobile/financial-transactions", h.MobileTypedTransaction)
	mux.HandleFunc("/api/mobile/parties", h.MobileListParties)
